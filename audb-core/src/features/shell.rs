...
pub async fn execute(as_root: bool, command: String) -> Result<()> {
    ...
    let output = if as_root {
        session.exec_as_root(&command).context(
            "Failed to execute command as root. Configure or update the cached password with: audb device set-root-password --new-password <value>",
        )?
    } else {
        session
            .exec(&command)
